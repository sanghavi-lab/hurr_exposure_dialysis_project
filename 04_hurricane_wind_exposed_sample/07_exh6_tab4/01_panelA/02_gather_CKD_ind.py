...
client = Client("10.50.87.74:37079")
print(client)
...
def read_cc_ckd_subset(year: int, bene_ids: pd.Series) -> pd.DataFrame:
    # Read only BENE_ID + CHRONICKIDNEY_EVER from MBSF CC file

    pq = mbsf_cc_path(year)
    if not _exists(pq):
        print(f"[WARN] {year}: missing CC path: {pq}")
    ...
    if ckd_col is None:
        print(f"[WARN] {year}: CHRONICKIDNEY_EVER not found in CC parquet.")
        return pd.DataFrame(columns=["BENE_ID", "CHRONICKIDNEY_EVER"])
    ...
    print(f"[INFO] {year}: CC matched bene-year rows = {len(x):,}")
