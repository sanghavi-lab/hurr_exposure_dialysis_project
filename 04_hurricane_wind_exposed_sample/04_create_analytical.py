...
# Description: This script takes the dialysis line item file from the previous step and turns it into a two-row analytical 
# file used for modeling: one row for the reference week (week_rel = -2) and one row for the exposure week (week_rel = 0). 
# It merges in dialysis-derived columns (like gap days, early dialysis indicator), ED, inpatient, and MBSF death flags and 
# then reshapes everything into a long beneficiary-storm panel for later within-beneficiary analyses.
#----------------------------------------------------------------------------------------------------------------------#
...
DROP_DUPLICATE_BENE_STORM_WEEK = True # final long panel removes duplicate rows for the same beneficiary-storm-week

# ... Window definitions (rel_day) ...
# Panel rows:
REF_WK_M2_START, REF_WK_M2_END = -14, -8   # week -2 (two weeks prior to exposure to hurricane)
PRE_WK_M1_START, PRE_WK_M1_END = -7, -1    # week -1 (one week prior to exposure to hurricane)
HAZ_WK0_START, HAZ_WK0_END     = 0, 6      # week 0 (week of exposure to hurricane)
POST_D1_D7_START, POST_D1_D7_END = 1, 7    # # Post-exposure dialysis window used in the early-dialysis analysis

# Additional weekly outcome windows
WK1_START, WK1_END = 7, 13 # week +1
WK2_START, WK2_END = 14, 20 # week +2
WK3_START, WK3_END = 21, 27 # week +3

# Cumulative post windows for pooled analysis
POST_2WK_START, POST_2WK_END = 0, 13   # weeks 0-1
POST_3WK_START, POST_3WK_END = 0, 20   # weeks 0-2
POST_4WK_START, POST_4WK_END = 0, 27   # weeks 0-3

# Schedule classification window (to identify if bene belongs to MWF or TTS schedules)
SCHED_START, SCHED_END = -21, -15

# Facility PRE window for PRVDR_NUM_event (covers schedule + reference week)
FAC_PRE_START, FAC_PRE_END = -21, -8 # to see which facility the bene belongs to

# Optional 2-week dialysis windows retained for QC / robustness / sensitivity checks
...
OUT_DIR  = os.path.join(OUT_BASE, "04_analytical_panel_hurr_exposure_v05_wkm2_facclust_cumpost_cumdeath")
...
def dialysis_features_from_step3_detail(events: pd.DataFrame, detail: pd.DataFrame) -> pd.DataFrame: # takes two pandas dataframes
    """
    Turns the line items from the previous script into beneficiary-storm level for identification of early dialysis
    
    Schedule inference window (i.e. MWF or TTS):
      - rel_day in [-21, -15] (week -3)

    Reference week:
      - week -2: rel_day in [-14, -8]
    ...
    d = _normalize_ids(detail) # Cleaning
    ...
    ev_key = events[key + ["event_id"]].copy()
    d = d.merge(ev_key, on=key, how="inner")

    d["rel_day"] = (d["REV_CNTR_DT"] - d["exposure_start_dt"]).dt.days.astype("int16") # Compute relative day from exposure
    ...
    op_day = d[["event_id", "BENE_ID", "exposure_start_dt", "REV_CNTR_DT", "rel_day", "PRVDR_NUM"]].drop_duplicates(
        subset=["event_id", "BENE_ID", "REV_CNTR_DT"],
        keep="first"
    )

    # Create week/window indicators
    op_day["wk_m2"]  = ((op_day["rel_day"] >= REF_WK_M2_START) & (op_day["rel_day"] <= REF_WK_M2_END)).astype("int8")
    op_day["wk_m1"]  = ((op_day["rel_day"] >= PRE_WK_M1_START) & (op_day["rel_day"] <= PRE_WK_M1_END)).astype("int8")
    op_day["wk0"]    = ((op_day["rel_day"] >= HAZ_WK0_START)   & (op_day["rel_day"] <= HAZ_WK0_END)).astype("int8")
    op_day["wk1"]    = ((op_day["rel_day"] >= WK1_START)       & (op_day["rel_day"] <= WK1_END)).astype("int8")
    op_day["wk2"]    = ((op_day["rel_day"] >= WK2_START)       & (op_day["rel_day"] <= WK2_END)).astype("int8")
    op_day["wk3"]    = ((op_day["rel_day"] >= WK3_START)       & (op_day["rel_day"] <= WK3_END)).astype("int8")
    op_day["post_d1_d7"] = ((op_day["rel_day"] >= POST_D1_D7_START) &(op_day["rel_day"] <= POST_D1_D7_END)).astype("int8")
    ...
    op_day["date_pre_ref"] = op_day["REV_CNTR_DT"].where(op_day["wk_m2"] == 1)
    op_day["date_pre_m1"]  = op_day["REV_CNTR_DT"].where(op_day["wk_m1"] == 1)
    op_day["date_post"]    = op_day["REV_CNTR_DT"].where(op_day["wk0"] == 1)
    ...
    grp = ( # Collapse to one row per event-beneficiary
        ...
        .agg(
            ...
            n_dialysis_wk0=("wk0", "sum"),
            n_dialysis_wk1=("wk1", "sum"),
            n_dialysis_wk2=("wk2", "sum"),
            n_dialysis_wk3=("wk3", "sum"),
            n_dialysis_post_d1_d7=("post_d1_d7", "sum"),
            n_dialysis_2wk_prior=("prior2", "sum"),
            n_dialysis_2wk_post=("post2", "sum"),
            date_pre_ref=("date_pre_ref", "max"),
            date_pre_m1=("date_pre_m1", "max"),
    ...
    # Compute gap between last pre-storm dialysis and first hazard-week dialysis
    grp["gap_days"] = np.nan
    valid_gap = grp["date_pre_m1"].notna() & grp["date_post"].notna() # Only people with both (1) a dialysis day in week -1 and (2) a dialysis day in week 0 can have a measurable gap.
    grp.loc[valid_gap, "gap_days"] = (
        grp.loc[valid_gap, "date_post"] - grp.loc[valid_gap, "date_pre_m1"]
    ).dt.days.astype("float") # compute the number of days from the last dialysis in week -1 to the first dialysis in week 0.
    grp["no_hazard_dialysis"] = grp["date_post"].isna().astype("int8") # Create a flag for whether the beneficiary had no dialysis at all in hazard week 0
    ...
    grp["pre_m1_dow"] = pd.to_datetime(grp["date_pre_m1"], errors="coerce").dt.weekday
    ...
    grp.loc[mwf_mask & (~grp["pre_m1_dow"].isin([0, 2, 4])), "earlyA_last_pre_offschedule"] = 1 # if someone is MWF, but their last week -1 dialysis day is not Monday, Wednesday, or Friday, flag them as early/off-schedule
    grp.loc[tts_mask & (~grp["pre_m1_dow"].isin([1, 3, 5])), "earlyA_last_pre_offschedule"] = 1 # if someone is TTS, but their last week -1 dialysis day is not Tuesday, Thursday, or Saturday, flag them too
...
def outcome_flags_from_ed_year(events: pd.DataFrame, ed_pq: str) -> pd.DataFrame:
    ...
    tmin = events["exposure_start_dt"].min() + pd.Timedelta(days=TWK_PRIOR_START) # earliest exposure date in the cohort, shifted backward to the start of the two-week pre window
    tmax = events["exposure_start_dt"].max() + pd.Timedelta(days=POST_4WK_END) # latest exposure date in the cohort, shifted forward to the end of the 4-week post window
    ...
    cohort_dd = dd.from_pandas(events[["event_id", "BENE_ID", "exposure_start_dt"]].copy(), npartitions=1)
    ...
    ed = ed.assign(rel_day=(ed["date"] - ed["exposure_start_dt"]).dt.days) # each ED-event pairing, compute the number of days between the ED date and the storm exposure date.

    ed = ed.assign( # creates window-specific indicators
        flag_wk_m2=((ed["rel_day"] >= REF_WK_M2_START) & (ed["rel_day"] <= REF_WK_M2_END)).astype("int8"),
        flag_wk0=((ed["rel_day"] >= HAZ_WK0_START) & (ed["rel_day"] <= HAZ_WK0_END)).astype("int8"),
        flag_wk1=((ed["rel_day"] >= WK1_START) & (ed["rel_day"] <= WK1_END)).astype("int8"),
        flag_wk2=((ed["rel_day"] >= WK2_START) & (ed["rel_day"] <= WK2_END)).astype("int8"),
        flag_wk3=((ed["rel_day"] >= WK3_START) & (ed["rel_day"] <= WK3_END)).astype("int8"),
...
def outcome_flags_from_ip_year(events: pd.DataFrame, medpar_pq: str) -> pd.DataFrame:
    # This process for IP is VERY similar to ED above. Please see comments above (under function outcome_flags_from_ed_year()) for more details
    if events.empty:
        return pd.DataFrame(columns=[
            "event_id", "BENE_ID",
    ...
    tmin = events["exposure_start_dt"].min() + pd.Timedelta(days=TWK_PRIOR_START)
    tmax = events["exposure_start_dt"].max() + pd.Timedelta(days=POST_4WK_END)
    ...
    ip = ip.assign(rel_day=(ip["ADMSN_DT"] - ip["exposure_start_dt"]).dt.days)

    ip = ip.assign(
        flag_wk_m2=((ip["rel_day"] >= REF_WK_M2_START) & (ip["rel_day"] <= REF_WK_M2_END)).astype("int8"),
        flag_wk0=((ip["rel_day"] >= HAZ_WK0_START) & (ip["rel_day"] <= HAZ_WK0_END)).astype("int8"),
        flag_wk1=((ip["rel_day"] >= WK1_START) & (ip["rel_day"] <= WK1_END)).astype("int8"),
        flag_wk2=((ip["rel_day"] >= WK2_START) & (ip["rel_day"] <= WK2_END)).astype("int8"),
        flag_wk3=((ip["rel_day"] >= WK3_START) & (ip["rel_day"] <= WK3_END)).astype("int8"),
...
def build_long_panel(
    ...
    if not dial.empty:
        dial = dial.copy()
        dial["BENE_ID"] = _as_clean_str(dial["BENE_ID"]) # clean
    ...
    df["disrupt_wk_m2"] = (df["n_dialysis_wk_m2"] < 3).astype("int8") # week -2
    df["disrupt_wk0"]   = (df["n_dialysis_wk0"]   < 3).astype("int8") # week of exposure
    df["disrupt_wk1"]   = (df["n_dialysis_wk1"]   < 3).astype("int8") # week 1, etc...
    df["disrupt_wk2"]   = (df["n_dialysis_wk2"]   < 3).astype("int8")
    df["disrupt_wk3"]   = (df["n_dialysis_wk3"]   < 3).astype("int8")
    df["disrupt_post_d1_d7"] = (df["n_dialysis_post_d1_d7"] < 3).astype("int8") # fewer than 3 outpatient dialysis days during post-exposure days 1-7
    ...
    # There were used: cumulative post disruption is defined as any disruption in any week inside that window.
    df["disrupt_post_2wk"] = (
        (df["disrupt_wk0"] == 1) |
        (df["disrupt_wk1"] == 1)
    ).astype("int8")
    ...
    for c in outcome_fill_cols:
        if c not in df.columns:
            df[c] = 0
        df[c] = df[c].fillna(0).astype("int8") # clean
    ...
    died_before = df["BENE_DEATH_DT"].notna() & (df["BENE_DEATH_DT"] < df["exposure_start_dt"])
    n_before = len(df)
    df = df[~died_before].copy()
    if n_before - len(df) > 0:
        print(f"Year {year}: dropped {n_before-len(df):,} event rows with death before exposure date.")

    df["event_dow"] = df["exposure_start_dt"].dt.weekday.astype("int8") # Create the weekday of the storm exposure date: Monday = 0 Tuesday = 1, etc
    ...
    death_rel_day = (df["BENE_DEATH_DT"] - df["exposure_start_dt"]).dt.days # compute the number of days from exposure date to death date.

    # Should remain 0 after dropping deaths before exposure date
    df["any_death_wk_m2"] = ( # week -2
        death_rel_day.notna() &
        (death_rel_day >= REF_WK_M2_START) &
        (death_rel_day <= REF_WK_M2_END)
    ).astype("int8")

    # Cumulative dead-by-week indicators from week 0 onward
    df["any_death_wk0"] = (
        death_rel_day.notna() &
        (death_rel_day >= HAZ_WK0_START) &
        (death_rel_day <= HAZ_WK0_END)
    ).astype("int8")

    df["any_death_wk1"] = (
        death_rel_day.notna() &
        (death_rel_day >= HAZ_WK0_START) &
        (death_rel_day <= WK1_END)
    ).astype("int8")

    df["any_death_wk2"] = (
        death_rel_day.notna() &
        (death_rel_day >= HAZ_WK0_START) &
        (death_rel_day <= WK2_END)
    ).astype("int8")

    df["any_death_wk3"] = (
        death_rel_day.notna() &
        (death_rel_day >= HAZ_WK0_START) &
        (death_rel_day <= WK3_END)
    ).astype("int8")
    ...
    base_cols = [
        "year","event_id","BENE_ID","storm_id","storm_year","fips","exposure_start_dt",
        "event_dow","schedule_type","stable_3x_weekly","earlyA_last_pre_offschedule",
        "gap_days","no_hazard_dialysis",
        "PRVDR_NUM_event",
        "BENE_DEATH_DT","BENE_BIRTH_DT","SEX_IDENT_CD",
    ] # These are the columns that should appear in both the week -2 row and the week 0 row before adding window-specific outcomes.

    # --- Week -2 row ---
    wk_m2 = df[base_cols].copy()
    wk_m2["week_rel"] = -2 # Mark this row as the reference week 
    ...
    wk_m2["any_death"] = df["any_death_wk_m2"].values # Again, this should be zero since we dropped those who died prior. More of a sanity check.
    ...
    wk0["week_rel"] = 0 # Mark this as week 0 (week of exposure)
    ...
    wk0["earlyA_last_pre_offschedule"] = df["earlyA_last_pre_offschedule"].fillna(0).astype("int8").values # Fill if bene received early dialysis
    ...
    long = pd.concat([wk_m2, wk0], ignore_index=True) # Concat so each bene-storm event gets two rows

    if (long.loc[long["week_rel"] == -2, "any_death"] == 1).any(): # QC
        raise ValueError("Found any_death==1 in week_rel=-2; check death-before-event drop logic.")
...
if __name__ == "__main__":
    for year in range(YEAR_MIN, YEAR_MAX + 1):
        print(f"\n=== STEP 4 v05: Processing storm_year={year} ===")
        ...
        long = build_long_panel(year, events, ip_out, ed_out, dial, mbsf) # create two rows for each bene - (1) ref row and (2) exposure row
        if long.empty:
            print(f"[SKIP] {year}: produced empty panel")
            continue
