...
def run_within_bene_demean(
    ...
    *,
    ...
    and hazard-week indicator within each BENE_ID × storm_id pair.
    ...
    # Keep rows with the identifiers and variables needed for the model.
    d = df.dropna(
        subset=["BENE_ID", STORM_COL, "hazard_week", y_col]
    ).copy()
    ...
    d["hazard_week"] = pd.to_numeric(
        d["hazard_week"], errors="coerce"
    ).astype(float)

    d[y_col] = pd.to_numeric(
        d[y_col], errors="coerce"
    ).astype(float)
    ...
    if np.isclose(x_dm.var(ddof=0), 0.0):
        return None, None, "no_within_bene_storm_variation_in_hazard"
