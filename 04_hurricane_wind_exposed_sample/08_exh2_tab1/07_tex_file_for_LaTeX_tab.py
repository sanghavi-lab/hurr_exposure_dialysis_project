...
INPUT_BASE = (
    ...
    "05g_analytical_panel_hurr_exposure_v05_wkm2_facclust_cumpost_cumdeath_plus_comorb_v01_"
    "plus_ahrf_v01_plus_mbsf_demo_cc_otcc_v01_plus_wind_v01"
)

OUT_DIR = (
    "/gpfs/data/cms-share/duas/54200/Jessy/data/climate_change/dialysis/"
    "table1_early_vs_nonearly_v04_ckd_only"
)
os.makedirs(OUT_DIR, exist_ok=True)

OUT_MACRO_TEX = os.path.join(OUT_DIR, "table1_county_macros.tex")
OUT_EVENT_CSV = os.path.join(OUT_DIR, "table1_event_level_dataset.csv")
OUT_CKD_EXPORT_CSV = os.path.join(OUT_DIR, "event_ckd_flag_export.csv")
...
def nice_label(var: str) -> str:
    raw = str(var).replace("__otcc", "").replace("_medicare", "").strip() # removes suffixes
...
for year in range(YEAR_MIN, YEAR_MAX + 1):
    ...
    if "event_id" not in df.columns:
        raise ValueError(f"{year}: missing event_id in input file.")

    dup_event = df.duplicated(subset=["event_id"]).sum() # QC check. If week_rel == 0 truly gives one row per event, then each event_id should appear only once.
    if dup_event > 0:
        raise ValueError(
            f"{year}: found {dup_event:,} duplicated event_id rows after restricting to week_rel == 0."
        )
...
print(f"[INFO] event-level rows after stacking all years = {len(event_df):,}")
print(f"[INFO] unique event_id = {event_df['event_id'].nunique(dropna=True):,}")

# ... Clean / derive some analysis variables (like age) for Table 1...

for c in ["BENE_ID", "storm_id", "fips"]:
...
event_df["exposure_start_dt"] = pd.to_datetime(event_df["exposure_start_dt"], errors="coerce")
event_df["BENE_BIRTH_DT"] = pd.to_datetime(event_df["BENE_BIRTH_DT"], errors="coerce")

event_df["age"] = ( # create age
    (event_df["exposure_start_dt"] - event_df["BENE_BIRTH_DT"]).dt.days / 365.25
)
...
print(f"[INFO] kept {len(event_df):,} events with {GROUP_VAR} in {{0,1}}; dropped {before_n - len(event_df):,}")
...
exclude_cols = { # set of variables that should not be scanned as candidate binary condition indicators.
    ...
    # baseline variables already handled explicitly elsewhere
    ...
    "exposure_start_dt", "event_dow", "BENE_DEATH_DT", "BENE_BIRTH_DT",
    ...
    "n_dialysis_wk_m2", "n_dialysis_wk0", "n_dialysis_2wk_prior", "n_dialysis_2wk_post",
    "disrupt_wk_m2", "disrupt_wk0", "disrupt_2wk_prior", "disrupt_2wk_post",
}
...
event_ckd_export = event_df[
    ["year", "storm_id", "event_id", "BENE_ID", "fips", "PRVDR_NUM_event"]
...
event_ckd_export.to_csv(OUT_CKD_EXPORT_CSV, index=False)
print(f"[WROTE] CKD flag export: {OUT_CKD_EXPORT_CSV}")
...
print(f"[INFO] kept {len(event_df):,} CKD events; dropped {before_ckd_n - len(event_df):,}")

if len(event_df) == 0:
    raise ValueError("After restricting to CKD == 1, no events remain.")
...
print(f"[WROTE] event-level table dataset: {OUT_EVENT_CSV}")
...
rows_tex.append(macro_line("N", f"{fmt_int(len(g1))} & {fmt_int(len(g0))}", "N")) # sample size
...
with open(OUT_MACRO_TEX, "w", encoding="utf-8") as f:
    f.write("% Auto-generated LaTeX macros for county-exposed beneficiary sample\n")
