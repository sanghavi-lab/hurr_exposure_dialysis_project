...
/* List years to process (no data for 2013, 2015, or 2022) */
%let year_list =
    2011
    2012
    2014
...
%do %while(%length(&this_year));
    ...
    /* ---------- READ-IN ---------- */
    proc import
        datafile="/gpfs/data/cms-share/duas/54200/Jessy/data/climate_change/dialysis/05b_comorbidity_prep_for_sas_from_step4_v01_csv/year_&this_year..csv"
        out      = diag_imp
        replace  ;
    ...
    /* Convert Dx_CodeType to 2-character string */
    data diag_imp;
        set diag_imp;
        length charvar_cd_tp $2.;
        charvar_cd_tp = strip(put(Dx_CodeType, z2.));
        drop Dx_CodeType;
    ...
    proc export
        data    = &score_set
        dbms    = csv
        outfile = "/gpfs/data/cms-share/duas/54200/Jessy/data/climate_change/dialysis/05c_comorbidity_scores_from_sas_v01/year_&this_year._sas_output.csv"
        replace;
    run;

    /* ---------- CLEAN WORK TABLES ---------- */
    proc datasets library=work nolist;
        delete diag_imp for_combined_comorbidity_score combined_comorb_score_set;
    quit;
