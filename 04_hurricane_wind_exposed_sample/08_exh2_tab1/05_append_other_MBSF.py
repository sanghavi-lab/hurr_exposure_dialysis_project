...
client = Client("10.50.87.106:36285")
print(client)
...
OUT_BASE = (
    "/gpfs/data/cms-share/duas/54200/Jessy/data/climate_change/dialysis/"
    "05f_analytical_panel_hurr_exposure_v05_wkm2_facclust_cumpost_cumdeath_plus_comorb_v01_plus_ahrf_v01_plus_mbsf_demo_cc_otcc_v02"
)
...
def make_demo_indicators(m: pd.DataFrame, year: int, analytical: pd.DataFrame) -> pd.DataFrame:
    ...
    ev = (
        analytical[["event_id", "BENE_ID", "exposure_start_dt"]]
    ...
    ev["exposure_start_dt"] = pd.to_datetime(ev["exposure_start_dt"], errors="coerce").dt.normalize()
    ev["exposure_month"] = ev["exposure_start_dt"].dt.month.astype("Int64")

    tmp = ev.merge(m, on="BENE_ID", how="left", indicator=True)

    print("[QC] event crosswalk <- MBSF ABCD merge:")
    ...
    tmp.loc[
        tmp["dual_code_at_exposure_month"].isin(dual_no) |
        tmp["dual_code_at_exposure_month"].isna(),
        "dual"
    ] = 0
    ...
    # for any recognized code, default to 0 first
    ...
    if dup_event > 0:
        raise ValueError(f"{year}: event-level demo output has {dup_event:,} duplicated event_id values.")
...
def make_cc_indicators(cc_df: pd.DataFrame, analytical: pd.DataFrame, source_label: str) -> pd.DataFrame:
    ...
    indicator = 1 if ever_date <= exposure_start_dt else 0
    """
    ev = (
        analytical[["event_id", "BENE_ID", "exposure_start_dt"]]
        .drop_duplicates(subset=["event_id"])
    ...
    print(f"[QC] event crosswalk <- {source_label} merge:")
    print(tmp["_merge"].value_counts(dropna=False).to_string())
    tmp = tmp.drop(columns=["_merge"])
    ...
    for raw_col in cc_cols:
        ...
        ind = ((dt.notna()) & (dt <= tmp["exposure_start_dt"])).astype("int8")
        out[out_col] = ind

    dup_event = out.duplicated(subset=["event_id"]).sum()
    if dup_event > 0:
        raise ValueError(f"event-level {source_label} output has {dup_event:,} duplicated event_id values.")

    print(f"[INFO] {source_label} indicators created = {len([c for c in out.columns if c not in ['event_id','BENE_ID']]):,}")
    return out

def qc_event_consistency(df: pd.DataFrame, event_id_col: str, varlist):
    bad_total = 0
    for c in varlist:
        chk = df.groupby(event_id_col, dropna=False)[c].nunique(dropna=False)
        bad_total += (chk > 1).sum()
    return bad_total
...
def process_year(year: int):
    ...
    required = ["event_id", "BENE_ID", "exposure_start_dt"]
    for c in required:
        if c not in analytical.columns:
            raise ValueError(f"{year}: analytical file must contain {c}.")
    ...
    analytical["exposure_start_dt"] = pd.to_datetime(analytical["exposure_start_dt"], errors="coerce").dt.normalize()
    ...
    demo_event = make_demo_indicators(abcd, year, analytical)
    print(f"[INFO] event-level demo rows = {len(demo_event):,}")

    # -------------------------
    # Read CC + OTCC
    # -------------------------
    cc = read_cc_like_subset(year, analytical["BENE_ID"], mbsf_cc_path(year), "CC")
    ...
    cc_event = make_cc_indicators(cc, analytical, "CC")
    otcc_event = make_cc_indicators(otcc, analytical, "OTCC")

    # guard against overlapping output names between CC and OTCC
    cc_vars = [c for c in cc_event.columns if c not in ["event_id", "BENE_ID"]]
    otcc_vars = [c for c in otcc_event.columns if c not in ["event_id", "BENE_ID"]]
    overlap = sorted(set(cc_vars).intersection(set(otcc_vars)))
    if overlap:
        rename_map = {c: f"{c}__otcc" for c in overlap}
        otcc_event = otcc_event.rename(columns=rename_map)
    ...
    # -------------------------
    event_all = demo_event.merge(cc_event, on=["event_id", "BENE_ID"], how="left")
    event_all = event_all.merge(otcc_event, on=["event_id", "BENE_ID"], how="left")
    ...
    print("[QC] analytical <- event_all merge:")
    ...
    if n_events_after != n_events_before:
        raise ValueError(
            f"{year}: unique event count changed after MBSF merge. Before={n_events_before:,}, After={n_events_after:,}"
        )

    # -------------------------
    # QC new vars
    # -------------------------
    new_vars = [c for c in event_all.columns if c not in ["event_id", "BENE_ID"]]
    ...
    bad_repeat = qc_event_consistency(analytical2, "event_id", new_vars_present)
    if bad_repeat > 0:
        raise ValueError(
            f"{year}: found {bad_repeat:,} event-by-variable inconsistencies in appended MBSF values."
        )
