...
WIND_FILE = (
    ...
    "brooke_hurricane/update_ethan_2025/storm_winds.csv"
)

OUT_BASE = (
    "/gpfs/data/cms-share/duas/54200/Jessy/data/climate_change/dialysis/"
    "05g_analytical_panel_hurr_exposure_v05_wkm2_facclust_cumpost_cumdeath_plus_comorb_v01_plus_ahrf_v01_plus_mbsf_demo_cc_otcc_v01_plus_wind_v01"
)
...
def process_year(year: int):
    ...
    for c in ["event_id", "storm_id", "fips"]:
    ...
    bad_events = (rows_per_event != 2).sum() # should be two rows per bene-storm (event)
    print(f"[QC] event_id with !=2 panel rows = {bad_events:,}") # print if any rows do not have two
    ...
    if len(analytical2) != n_rows_before: # QCs
        raise ValueError(
    ...
    if n_events_after != n_events_before: # QCs
        raise ValueError(
            f"{year}: unique event count changed after wind merge. "
    ...
    event_missing = (
        analytical2[["event_id", "wind_missing"]]
        .drop_duplicates(subset=["event_id"])
        .copy()
    )
    n_missing_events = event_missing["wind_missing"].sum()
    ...
    print(f"[QC] events missing vmax_sust = {n_missing_events:,} / {len(event_missing):,} ({pct_missing_events:.3%})")

    # ... QC: wind should not vary within bene_storm (event) ...
    chk = analytical2.groupby("event_id", dropna=False)["vmax_sust"].nunique(dropna=False)
    bad_repeat = (chk > 1).sum()
    if bad_repeat > 0:
        raise ValueError(
            f"{year}: found {bad_repeat:,} event_id values with inconsistent vmax_sust across panel rows."
        )
