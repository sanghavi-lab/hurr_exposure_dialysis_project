...
def fit_ols_cluster_one_or_two_way( # runs the OLS model
    ...
    # This is for if we want one-way clustering (for storm-specific analyses primarily)
    if g2 is None:
        try:
            g1s = g1v.astype(str)
            res_c = model.fit(
    ...
    try:
        ...
        # cov_cluster_2groups returns:
        # 1. combined two-way clustered covariance
        # 2. covariance clustered by the first group only
        # 3. covariance clustered by the second group only
        cov_both, cov_g1, cov_g2 = cov_cluster_2groups(
            res_classic,
            g1_codes,
            g2_codes,
        )
    
        # Use only the combined two-way clustered covariance matrix.
...
def run_within_bene_demean(
    ...
    *,
    ...
    and hazard-week indicator within each year × event_id × BENE_ID unit. (Bene-storm)
    ...
    # ----------------------------------------------------------
    # Within-beneficiary-event (bene-storm essentially) transformation
    # ----------------------------------------------------------
    g = d.groupby(
        [
            "year",
...
def _single_result(df_block: pd.DataFrame, y_col: str, *, pooled: bool) -> dict: # used to run one model for one outcome
    ...
    ) # run the within-bene-storm model.
...
def compute_stacked_pooled_results(df_all: pd.DataFrame) -> list[dict]: # create the pooled table rows
    ...
    cumulative_specs = [
        (
            "Week -7 vs cumulative weeks -5 to -4",
            "disrupt_cmp_2wk",
            "any_ed_cmp_2wk",
            "any_ip_cmp_2wk",
            "any_death_cmp_2wk",
        ...
        (
            "Week -7 vs cumulative weeks -5 to -3",
            "disrupt_cmp_3wk",
            "any_ed_cmp_3wk",
            "any_ip_cmp_3wk",
            "any_death_cmp_3wk",
        ...
        (
            "Week -7 vs cumulative weeks -5 to -2",
            "disrupt_cmp_4wk",
            "any_ed_cmp_4wk",
            "any_ip_cmp_4wk",
            "any_death_cmp_4wk",
...
def main():
    ...
    pooled_rows = compute_stacked_pooled_results(df_all) # run the pooled within-bene-storm models.
