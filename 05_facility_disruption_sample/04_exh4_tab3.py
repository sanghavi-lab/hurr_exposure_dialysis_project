...
# restricts the sample to chronic kidney disease (CKD) beneficiaries, and estimates within-beneficiary-storm changes in disruption, 
...
OUTPUT_DIR = (
    "/gpfs/data/cms-share/duas/54200/Jessy/data/climate_change/dialysis/"
    "paper_exhibits_step5_disrupted_sample/"
)
...
OUT_TEX_POOLED_BODY = os.path.join(
    OUTPUT_DIR, "exhibit_step5_disrupted_pooled_multihorizon_stacked_body_wkm2.tex"
)
OUT_TEX_POOLED_FULL = os.path.join(
    OUTPUT_DIR, "exhibit_step5_disrupted_pooled_multihorizon_stacked_full_wkm2.tex"
)

OUT_TEX_STORM_BODY = os.path.join(
    OUTPUT_DIR, "exhibit_step5_disrupted_bystorm_weekly_body_wkm2.tex"
)
OUT_TEX_STORM_FULL = os.path.join(
    OUTPUT_DIR, "exhibit_step5_disrupted_bystorm_weekly_full_wkm2.tex"
)

OUT_CSV_DISRUPT = os.path.join(OUTPUT_DIR, "storm_coefs_disrupted_disruption_pp_weeklyonly_wkm2.csv")
OUT_CSV_ED      = os.path.join(OUTPUT_DIR, "storm_coefs_disrupted_ed_pp_weeklyonly_wkm2.csv")
OUT_CSV_IP      = os.path.join(OUTPUT_DIR, "storm_coefs_disrupted_ip_pp_weeklyonly_wkm2.csv")
OUT_CSV_DEATH   = os.path.join(OUTPUT_DIR, "storm_coefs_disrupted_mortality_pp_weeklyonly_wkm2.csv")
...
def n_paired_event_bene(df: pd.DataFrame) -> int: # returns a count of unique beneficiary-event (i.e., bene-storm) combinations
    if df.empty:
        return 0
...
def run_within_bene_demean(
    ...
    *,
    ...
    and hazard-week indicator within each event_id × BENE_ID pair.
    ...
    # ----------------------------------------------------------
    # Within-beneficiary-event transformation
    # ----------------------------------------------------------
    g = d.groupby(
        ["event_id", "BENE_ID"],
        sort=False,
    ...
    if np.isclose(x_dm.var(ddof=0), 0.0):
        return (
            None,
            None,
            "no_within_event_bene_variation_in_hazard",
        )
    ...
    if use_two_way_if_possible:
        ...
        res, cov, status = fit_ols_cluster_one_or_two_way(
            y_dm,
            X,
            g1=g1,
            g2=g2,
...
def write_pooled_tex_files(body: str):
    ...
    full.append(rf"% Note c: Obs N is the number of paired event--bene units (unique (event_id, BENE_ID)) after enforcing both weeks ({REF_WEEK} and {HAZ_WEEK}). Obs N is shown only on the first pooled row because it is repeated across horizons.")
    ...
    full.append(rf"% Beneficiaries who died before anchor_dt were already excluded upstream.")
...
def write_storm_tex_files(body: str):
    ...
    full.append(r"% Note a: Effects reported in percentage points (coef and CI multiplied by 100).")
    full.append(r"% Note b (manual): Disruption estimated on stable MWF/TTS only (stable_3x_weekly==1 and schedule_type in {MWF,TTS}).")
    full.append(rf"% Note c: Obs N is the number of paired event--bene units (unique (event_id, BENE_ID)) after enforcing both weeks ({REF_WEEK} and {HAZ_WEEK}).")
...
def get_storm_category_and_year(storm_id: str) -> tuple[float, float]:
    """
    Return the highest U.S. Saffir-Simpson category and year for a storm.

    Category is obtained from STORM_CATEGORY.
    Year is extracted from the storm_id, such as Matthew-2016.
    """
...
def get_storm_ids_by_category(df_all: pd.DataFrame) -> list[str]:
    """
    Order storms by:
    ...
    order = order.sort_values(
        ["storm_category", "storm_year", STORM_COL],
...
def main():
    ...
    for c in ["BENE_ID", "storm_id", "event_id", "facility_county_fips", "facility_id"]:
    ...
    if "facility_id" in df_all.columns:
        df_all["facility_id"] = _clean_str_series(df_all["facility_id"])
    ...
    ckd_keep = ckd_df[
        ["year", "storm_id", "event_id", "BENE_ID", "facility_county_fips", "facility_id", "ckd_ind"]
    ].drop_duplicates().copy()

    df_all = df_all.merge(
        ckd_keep,
        on=["year", "storm_id", "event_id", "BENE_ID", "facility_county_fips", "facility_id"],
        how="left",
    ...
    pre_ckd_paired_event_bene = n_paired_event_bene(df_all)

    print(f"[INFO] Before CKD restriction: unique beneficiaries across all storms = {pre_ckd_unique_benes:,}")
    print(f"[INFO] Before CKD restriction: paired event-bene N = {pre_ckd_paired_event_bene:,}")
    ...
    print(
        ...
        f"paired event-bene N={n_paired_event_bene(df_all):,} | "
    ...
    if STORM_COL not in df_all.columns:  # check clustering cols
        raise KeyError(f"{STORM_COL} not found in disrupted sample panel.")

    if CLUSTER_COL not in df_all.columns: # check clustering cols
        raise KeyError(f"{CLUSTER_COL} not found in disrupted sample panel.")
    ...
    for sid in storm_ids:
        ...
        storm_results = compute_weekly_results_block(
            d,
            pooled=False,
        )

        # Show the storm category directly in the LaTeX table.
        ...
        for outcome in [
            ...
            csv_rows[outcome].append({
                ...
                "note": (
                    "weekly only; pp=coef*100"
                    if REPORT_IN_PERCENT_POINTS
                    else "weekly only"
                ),
            })

    # Write storm LaTeX file.
    storm_body = "".join(lines)
    write_storm_tex_files(storm_body)

    # Write storm-specific CSV files in the same order as the table.
    def _write_csv(
        out_path: str,
        rows: list[dict],
    ):
