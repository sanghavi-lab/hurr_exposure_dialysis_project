...
# beneficiary-storm models of ED, IP, and mortality outcomes on exposure week and early dialysis indicators, using two-way 
...
# before the storm is associated with different post-exposure health outcomes within beneficiary-storm.
#----------------------------------------------------------------------------------------------------------------------#
...
OUT_STATA_ANALYSIS = os.path.join(
    OUTPUT_DIR,
    f"facility_disruption_two_stage_analysis_ready_{SUFFIX}.dta",
)
...
OUTCOMES_ORDER = [
    ...
    ("any_ip", "IP"),
    ("any_death", "Death"),
    ("disrupt_cmp_post_d1_d7", "Disruption"),
]
...
DROP_STORM_IDS = {"Maria-2017"}
MAX_STORM_YEAR_KEEP = 2022 # Keep if 2022 or less
...
def get_storm_category_and_year(
    ...
    parsed_year = _parse_storm_year(sid)
...
def storm_sort_key(storm_id: str) -> tuple:
    """
    Sort by category, year, and storm name.

    Missing categories or years are placed at the end.
    """
    category, year = get_storm_category_and_year(
        storm_id
    )
...
def storm_is_kept(storm_id: str) -> bool:
    ...
    if storm_year is None: # if no valid year could be extracted, do not keep it.
        return False
    return storm_year <= MAX_STORM_YEAR_KEEP # keep the storm only if its year is less than or equal to the cutoff year 2022
...
def run_fe_panelols(
    ...
    # ----------------------------------------------------------
    # Create one entity for each event-beneficiary pair.
    # ----------------------------------------------------------
    d["_event_bene_id"] = (
        d.groupby(
            [EVENT_COL, BENE_COL],
    ...
    # Sort the reference and hazard observations within each entity.
    d = d.sort_values(
        ["_event_bene_id", WEEK_COL]
    ).copy()

    # Artificial time index within each event-beneficiary entity.
    d["_t"] = (
        d.groupby("_event_bene_id", sort=False)
        .cumcount()
    )

    # First index: event-beneficiary entity
    # Second index: observation order within the entity
    d = d.set_index(["_event_bene_id", "_t"])
    ...
    # Rough residual-degrees-of-freedom check.
    nobs = int(len(d))

    n_entities = int(
        d.index
        .get_level_values("_event_bene_id")
        .nunique()
    )

    k = int(X.shape[1])

    approx_df_resid = nobs - n_entities - k

    if approx_df_resid <= 0:
        return None, (
            f"infeasible_fe(df_resid≈{approx_df_resid}, "
            f"nobs={nobs}, n_entities={n_entities}, k={k})"
        )
    ...
    try:
        ...
        return None, (
            "infeasible_fe("
            f"zero_division_in_fit, nobs={nobs}, "
...
def filter_storm_rows(
    df_wide: pd.DataFrame,
) -> pd.DataFrame:
    """
    Filter and format the storm-specific output table
    for the appendix.

    Storms are ordered by:
      1. Lowest category first
    ...
    metadata = out["storm_id"].apply(
        get_storm_category_and_year
    )

    out["storm_category"] = metadata.apply(
        lambda x: x[0]
    )

    out["storm_year"] = metadata.apply(
        lambda x: x[1]
    )

    keep = out["storm_id"].apply(
        storm_is_kept
    )
    ...
    out["storm_display"] = out.apply(
        lambda r: (
            f"{r['storm_id']} "
            f"(Cat. {int(r['storm_category'])})"
            if pd.notna(r["storm_category"])
            else str(r["storm_id"])
        ),
        axis=1,
    )
...
def build_latex_body_pooled(df_wide: pd.DataFrame) -> str:
    ...
    row = [
        ...
        latex_escape(get_eff(r, "Disruption")),
    ]
    return " & ".join(row) + r" \\" + "\n"
...
def main() -> None:
    ...
    # Preserve identifiers and geographic/facility codes as strings.
    stata_id_cols = [
        BENE_COL,                  # BENE_ID
        STORM_COL,                 # storm_id
        EVENT_COL,                 # event_id
    ...
    # Confirm that every Stata variable name is within the 32-character limit.
    long_stata_names = [
        c for c in stata_df.columns
        if len(c) > 32
    ]
    
    if long_stata_names:
        raise ValueError(
            "The following variable names exceed Stata's "
            f"32-character limit: {long_stata_names}"
        )
    
    # Convert pandas extension types that may not export cleanly.
    for c in stata_df.columns:
        ...
        # Pandas string extension type
        elif dtype_name == "string":
            stata_df[c] = stata_df[c].astype(object)
    
    # Stata cannot export object columns containing pd.NA cleanly.
    # Replace pd.NA with None in string/object columns.
    for c in stata_df.select_dtypes(include=["object"]).columns:
        stata_df[c] = stata_df[c].where(
            stata_df[c].notna(),
            None,
        )
    ...
    print(
        ...
        f"  rows={len(stata_df):,}\n"
        f"  paired event-beneficiary units="
        f"{count_paired_event_bene_units(stata_df):,}\n"
        f"  unique beneficiaries="
        f"{stata_df[BENE_COL].nunique():,}\n"
        f"  storms="
        f"{stata_df[STORM_COL].nunique():,}\n"
        f"  facilities="
        f"{stata_df[CLUSTER_COL].nunique():,}"
    )
    ...
    print("[QC] storm_id top values:\n", df_all[STORM_COL].value_counts(dropna=False).head(20))
    ...
    for _, short in OUTCOMES_ORDER:  # create formatted display columns like: 1.2 [0.3, 2.1]
    ...
    pooled_notes = [
        ...
        r"% Note c: Reported effect is the coefficient on earlyA_last_pre_offschedule from y ~ hazard_week + earlyA + FE(event_id x BENE_ID).",
        r"% Current builder alignment: week_rel=-2 is reference and week_rel=0 is hazard.",
        r"% Dialysis disruption compares relative days -14 through -8 with post-exposure days 1 through 7.",
        r"% Disruption is defined as fewer than 3 outpatient dialysis days during the relevant 7-day window.",
        r"% Dialysis received on or before the exposure date is excluded from the post-exposure days 1--7 disruption outcome.",
        r"% The days 1--7 definition applies to dialysis disruption only; ED, IP, and mortality retain their existing outcome windows.",
    ...
    for sid in storms_present: # loop through one storm at a time and subsets to that storm.
        ...
        row = {
            ...
            "storm_display": storm_display,
            "n_pairs": int(
                count_paired_event_bene_units(df_s)
            ),
            "n_rows": int(len(df_s)),
            "n_bene": int(
                df_s[BENE_COL].nunique()
            ),
        }
    ...
    for _, short in OUTCOMES_ORDER: # add formatted percentage-point effect.
    ...
    if (
        not df_storm_long.empty
        and not df_storm_wide.empty
    ):
        keep_sids = set(
            df_storm_wide["storm_id"].tolist()
        )

        df_storm_long = df_storm_long[
            df_storm_long["storm_id"].isin(
                keep_sids
            )
        ].copy()
        ...
        outcome_order = {
            short: position
            for position, (_, short)
            in enumerate(OUTCOMES_ORDER)
        }

        df_storm_long["_outcome_order"] = (
            df_storm_long["outcome"]
            .map(outcome_order)
        )
    ...
    storm_notes = [
        r"% Within the storm-specific table, storms are ordered from the lowest to the highest U.S. Saffir-Simpson category, then chronologically within category, and alphabetically when storms share the same category and year.",
